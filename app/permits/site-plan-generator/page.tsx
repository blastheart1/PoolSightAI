  ...
  switch (action.type) {
    ...
    case "LOAD_INTERPRETATION":
      return {
        ...state,
        stage: "review",
        interpretation: action.saved.interpretation,
        confirmedFeatures: action.saved.confirmedFeatures,
        isLoading: false,
        error: null,
      };
    case "CONFIRM_FEATURE":
      return {
        ...state,
        confirmedFeatures: state.confirmedFeatures.map((f) =>
          f.id === action.featureId ? { ...f, engineerConfirmed: true } : f,
        ),
      };
    case "REMOVE_FEATURE":
      return {
        ...state,
        confirmedFeatures: state.confirmedFeatures.filter(
    ...
    case "EDIT_FEATURE":
      return {
        ...state,
        confirmedFeatures: state.confirmedFeatures.map((f) =>
          f.id === action.featureId ? { ...f, ...action.override } : f,
        ),
      };
    case "ADD_FEATURE":
      return {
        ...state,
        confirmedFeatures: [...state.confirmedFeatures, action.feature],
      };
    ...
    case "UPDATE_INPUTS":
      return {
        ...state,
        sitePlanInputs: { ...state.sitePlanInputs, ...action.inputs },
      };
    case "SET_OUTPUTS":
      return {
        ...state,
    ...
    async (file: File) => {
      try {
        ...
        if (!saved.version || !saved.interpretation || !saved.confirmedFeatures) {
          dispatch({
            type: "SET_ERROR",
            error: "Invalid saved interpretation file. Expected a JSON file previously exported from this tool.",
          });
          return;
        }
        ...
        dispatch({
          type: "SET_ERROR",
          error: "Could not parse the saved file. Make sure it is a valid JSON interpretation export.",
        });
      }
    },
    [],
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Upload a rendering or sketch → AI interprets → you confirm → generate
            SVG, DXF, and data sheet.
          </p>
        </div>
      ...
      <div className="flex gap-1">
        {(["upload", "review", "dimensions", "output"] as Stage[]).map(
          (s, i) => (
            <div
              key={s}
              className={`h-1 flex-1 rounded-full transition ${
                i <=
                ["upload", "review", "dimensions", "output"].indexOf(
                  state.stage,
                )
                  ? "bg-sky-500"
                  : "bg-slate-800"
      ...
      {state.stage === "upload" && (
        <div className="space-y-4">
          <label
            className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-700 bg-slate-900/40 py-16 transition hover:border-sky-600 hover:bg-slate-900/60"
          ...
          >
            ...
            <span className="text-sm font-medium text-slate-300">
              Drop a rendering, sketch, or PDF here
            </span>
          ...
          {state.imagePreviewUrl && (
            <div className="overflow-hidden rounded-lg border border-slate-800">
              ...
              <img
                ...
                alt="Uploaded rendering"
                className="max-h-[300px] w-full object-contain bg-slate-900"
              />
            </div>
          )}
          ...
          {!state.isLoading && (
            <label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-900/40 px-4 py-3 text-sm text-slate-300 transition hover:border-slate-500 hover:text-white">
              <svg
                className="h-4 w-4"
                fill="none"
                viewBox="0 0 24 24"
      ...
      {state.stage === "review" && state.interpretation && (
        <InterpretationReview
          ...
          onEdit={(id, override) =>
            dispatch({ type: "EDIT_FEATURE", featureId: id, override })
          }
          ...
          {state.outputs.dataSheet && (
            <div className="rounded-lg border border-slate-800 bg-slate-900/40 p-5">
              ...
              {state.outputs.dataSheet.engineerActionItems?.length > 0 && (
                <div>
                  <h4 className="mb-1 text-xs font-semibold uppercase tracking-wider text-amber-600">
                    Action Items
                  </h4>
                  <ul className="space-y-0.5 text-xs text-slate-300">
                    {state.outputs.dataSheet.engineerActionItems.map(
                      (item, i) => (
                        <li key={i} className="flex gap-2">
